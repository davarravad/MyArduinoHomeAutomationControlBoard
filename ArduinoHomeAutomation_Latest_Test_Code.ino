...
/***** Declare Constants *****/
// Debug Settings
#define DEBUG 1   // 1 For Debugging - 0 To Disable Debugging
...
String cdIN = "";
String cdINallDataLoop[NUM_BOARDS-1] = "";
String pageValueLight[] = "";

// Define Lights status
bool lightOutputValue[cdInputArraySize];
int lita = 0;
...
void setup(){
  ...
  Serial.begin(9600);
  Serial.println(F(" --------------------------------------------------- "));
  Serial.println(F(" | Arduino Home Automation v2.5"));
  Serial.println(F(" --------------------------------------------------- "));
  ...
  Serial.println(F(" | Designed by David Sargent"));
  Serial.println(F(" --------------------------------------------------- "));
  Serial.println(F(""));
  Serial.println(F(""));
  Serial.println(F(" --------------------------------------------------- "));
  Serial.println(F(" | Network Connection Information"));
  Serial.println(F(" --------------------------------------------------- "));
  ...
  if(internetEnabled == true){
    ...
    if (Ethernet.begin(mac) == 0){
      Serial.println(F(" | Failed to connect to local network.  Running in   "));
      Serial.println(F(" | No Internet Mode.  Website and other internet     "));
      Serial.println(F(" | devices will NOT work.  Please check your network "));
      Serial.println(F(" | and reset the Arduino controller.                 "));
      Serial.println(F(" --------------------------------------------------- "));
      Serial.println(F(""));
      Serial.println(F(""));
  ...
  Serial.println(F(" --------------------------------------------------- "));
  Serial.println(F(" | Setting up Shift registers"));
  Serial.println(F(" --------------------------------------------------- "));
  Serial.println(F(""));
  Serial.println(F(""));
  ...
  Serial.println(F(" --------------------------------------------------- "));
  Serial.println(F(" | Setting up array size for lightOutputValue"));
  Serial.println(F(" --------------------------------------------------- "));
  ...
  Serial.println(F(" --------------------------------------------------- "));
  Serial.println(F(" | Controller Setup Now Finished.  Moving on to loop."));
  Serial.println(F(" --------------------------------------------------- "));
  Serial.println(F(""));
  Serial.println(F(""));
  ...
  // Setup default values for CD chips in data for output to website
  for (int sbid=0; sbid<=NUM_BOARDS-1; sbid++)
  {
    cdINallDataLoop[sbid] = "0000000000000000";
  }

  // Setup default values to false for lightOutputValue[cdInputArraySize] array
  for (int cias=0; cias <= cdInputArraySize; cias++){
    lightOutputValue[cias] = 0; 
  }

  Serial.println(F(" --------------------------------------------------- "));
  Serial.print(F(" | lightOutputValue[0] : "));
  Serial.println(lightOutputValue[0]);
  Serial.println(F(" --------------------------------------------------- "));
...
void loop(){
  ...
  for (int bid=0; bid<=NUM_BOARDS-1; bid++)
  {
    if( DEBUG ) Serial.println();
    if( DEBUG ) Serial.println(" --------------------------------------------------- ");
    if( DEBUG ) Serial.print(" | cdINallDataLoop[");
    if( DEBUG ) Serial.print(bid);
    if( DEBUG ) Serial.print("] : ");
    if( DEBUG ) Serial.println(cdINallDataLoop[bid]);
    if( DEBUG ) Serial.println(" --------------------------------------------------- ");
    ...
    if(cdINallDataLoop[bid] != cdINallData[bid]){
    ...
    // Update Relay Loop
    cdINallDataLoop[bid] = cdINallData[bid];
    if( DEBUG ) Serial.println(" --------------------------------------------------- ");
    if( DEBUG ) Serial.print(" | cdINallDataLoop[");
    if( DEBUG ) Serial.print(bid);
    if( DEBUG ) Serial.print("] : ");
